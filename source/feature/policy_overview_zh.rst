...
1. 3个模式
    ...
    * ``learn_mode`` : 一组旨在更新和优化策略的函数/方法.
        ...
        下面是一个关于如何在串行(serial pipeline)中应用策略``learn_mode``的演示:

        .. image:: images/serial_learner.png
            :scale: 60%

    * ``collect_mode`` : 一组旨在探索(exploration)和利用(exploitation)平衡地收集训练数据的函数/方法。

        下面是一个关于如何在串行(serial pipeline)中应用策略``collect_mode``的演示:
    ...
    * ``eval_mode`` : 一组负责公平策略评估的功能/方法。
        ...
        下面是一个关于如何在串行(serial pipeline)中应用策略``eval_mode'`的演示:
...
3. 收集模式接口:
    ...
    * ``_process_transition`` : 将设想的时间步骤和策略输出处理成一个状态转移对(transition).

    * ``_get_train_sample`` : 从一连串的状态转移对(transition)中获取可用于训练的样本.
...
.. note::
    **如何在不同模式下定制模型？**
    ...
    在大多数情况下，学习、收集和评估模式使用一个相同的模型. 然而，他们可能会用不同的包装器(wrappers)来包装这个共享模型，以满足他们自己的需求. 比如说, 模型在收集和评估模式中不需要更新，而在学习模式中需要更新；收集模式模型可能需要使用探索(exploration)，而评估模式模型不需要.

    然而，在一些策略中，不同模式的模型是不同的. 例如，逆强化学习需要一个专家模式来收集专家数据(expert data)，然后用专家数据来训练一个新的模型.在这种情况下，用户需要在不同模式下定制模型.
