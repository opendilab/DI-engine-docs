...
该算法可以使智能体在没有奖励的环境中能够自我学习产生奖励函数, 并且自动探索出来一些有用的技能（skills）。这些学到的技能为很多RL任务提供了基石，例如，要找到一个能在任务中获得高回报的策略，通常只需选择具有最大回报的技能就足够了。
另外三个应用分别是：

1. 用于RL算法训练的策略初始化，加速学习过程。
2. 用于分层强化学习： 通过学习一个元控制器去组合DIAYN学习到的技能来解决复杂的分层任务。
3. 用于模仿学习：用学到的技能去模仿专家

DIAYN 是由三个核心的想法推导出来的。首先， 为了让学到的技能有用，我们想让技能去决定智能体在与环境交互中会经历的状态， 即不同的技能应该拜访不同的状态；其次，我们依靠状态，而不是动作去区分技能， 因为不对环境有影响的动作对于观察者来说是不可见的。
最后， 我们为了鼓励探索，并让学到的技能更加多元，对于每一个技能策略，我们尽可能地让他更随机的行动。
...
2. 训练策略的奖励函数是有判别器生成，而不是环境自带的奖励函数
   
3. DIAYN 是基于SAC算法训练的，其训练过程包括更新判别器以更好地预测技能，并更新技能以访问不同的状态，使其更具判别能力。
...
我们在episode的初始采集一个技能，并通过依附于这个技能的策略去跟环境交互产生数据。 通过产生的数据，我们去训练判别器使得其更好的通过状态判别技能。然后，我们更新策略使其产生出能让判别器的判别能力最大化的数据。
由于我们之前提到DIAYN的算法是无监督学习，所以训练策略的奖励函数是由上述三个核心想法计算得来的， 具体来说：
...
第一项鼓励我们对p(z)的先验分布具有很高的熵。在我们的方法中，我们把p(z)固定为均匀分布，保证它具有最大的熵。第二项表明，从当前状态推断技能z应该是很容易的。第三项表明，每个技能应该尽可能地随机行动，这一点我们通过使用最大熵策略来代表每个技能来实现。
由于我们无法对所有的状态和技能进行整合以准确计算p(z \| s)，我们用一个学习到的判别器q(z \| s)来近似这个后验。詹森不等式(Jensen’s Inequality) 告诉我们，用q(z \| s)代替p(z | s)可以给我们的目标F(θ)提供一个变分下限G(θ, φ)
...
由于在SAC算法训练中最大化了策略的熵， 所以我们只需要把奖励函数变成了如下形式并通过SAC算法训练策略，便等同于最大化G这个期望
...
注意我们在实现时也是采用了SAC中off-policy的形式，用mini-batch transition的模式来训练。其中discriminator 为一个神经网络，其输入是一个观测到的状态，输出skill的预测分布。


实现
---------------

DIAYN的接口定义如下,它相对于SAC主要有加入了判别器的训练流程及奖励函数的修改

.. autoclass:: ding.policy.diayn.DIAYNPolicy
   :noindex:

Note: ``...`` indicates the omitted code snippet. For the complete code, please refer to our
`implementation <https://github.com/opendilab/DI-engine/blob/main/ding/policy/diayn.py>`__ in DI-engine.
...
在收集数据时，我们会在每个episode前通过均匀分布采样一个技能
