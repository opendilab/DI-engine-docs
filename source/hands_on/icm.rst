...
算法试图解决的困难：
**1. 高维度的连续状态空间(比如图像信息）很难建立直观的dynamic model**，即  :math:`p_\theta(s_{t+1}, a_t)` ,原因是
在agent执行动作的过程中存在噪声，更重要的是，在一个stochasticity的系统中，环境的随机性也会影响建模的稳定性和准确性。
**２. 环境中的observation和agent自身行为的相关性有所区别**，大致可以分为：　
(1)agent可以直接控制的元素（比如自动驾驶场景中自车的位置，速度）；　
(2)虽然不受agent控制但是会对agent造成影响的元素（比如自动驾驶场景中他车的位置，速度）；　
(3)既不受agent控制又不会对agent造成影响的元素（比如自动驾驶场景中太阳光的强弱，虽然会对传感器造成影响，但是本质上不会影响驾驶行为）。
对于以上的三种类型的observation元素，我们希望提取第(1)(2)两种情境下的环境特征，（这两种环境特征与agent的动作相关），而忽略第(3)种特征（这种环境特征与agent的动作无关）。
...
3. 虽然逆向模型和前向模型都会参与loss的计算，但是只有前向模型会当做intrinsic reward。逆向模型并不作为intrinsic reward,它（逆向模型）的作用主要是为了在特征空间提取的过程中，更好地帮助表征与agent动作相关的环境特征。

4. 奖励归一化。 由于奖励是不稳定的，因此需要把奖励归一化到[0,1]之间，让学习更稳定。
