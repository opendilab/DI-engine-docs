...
这里我们展示了DI-engine设计的强化学习训练和评估的一些关键概念。
其中一个基本的控制模式（串行模式）可以描述为。

.. image::
   images/serial_pipeline.svg
   :align: center

在下面的章节中，DI-engine首先分别介绍了关键的概念/组件，然后像构建一个特殊的 "进化图 "一样将它们结合起来，以提供不同的计算模式。(serial模式, parallel模式, dist模式).

概念
----------
``Environment（环境）``和 ``policy（策略）``是整个设计方案中最重要的两个概念，也可以称为描述模块，在大多数情况下，DI-engine的用户只需要关注这两个部分。在大多数情况下，DI-engine的用户只需要注意这两个部分。

``Worker（工作者）``模块，如 ``learner（学习者）``, ``collector（收集者）``和 ``buffer（缓冲器）``，是执行从描述模块派生的相应任务的执行模块。这些工作模块在许多RL算法中是通用的，但是用户也可以很容易地覆盖它们的组件，唯一的限制是要遵守基本的接口定义。

最后，``config（配置文件）``是我们推荐的用于控制和记录整个训练过程的工具。

.. tip::
  环境和策略在其他RL文件和框架中的原始定义基础上进行了部分扩展。

环境
~~~~~~~~~~~~~
DI-engine环境是``gym.Env``的超集，它与gym env接口兼容，并提供一些可选的接口。例如: dynamic seed, collect/evaluate setting, `Env Overview <../feature/env_overview_en.html>`_

``EnvManager``, 通常在其他框架中被称为 "矢量环境"，旨在实现并行的环境模拟，以加快数据收集速度。它允许采集器在每个采集步骤中与N个同质环境交互，而不是只与1个环境交互，这意味着传递给`env.step'的`action'是一个长度为N的向量，而`env.step'的返回值（obs, reward, done）与它相同。

为了方便异步复位和统一异步/同步步骤，DI-engine这样修改了env manager的接口：
...
现在DI-engine中有三种类型的EnvManager：

  - BaseEnvManager——本地测试和验证
  - SyncSubprocessEnvManager——用于低波动环境的并行模拟
  - AsyncSubprocessEnvManager——用于高波动环境的并行模拟

下面的演示图显示了 "BaseEnvManager "和 "SyncSubprocessEnvManager "之间的详细运行时逻辑。

.. image::
   images/env_manager_base_sync.png

对于子进程类型的环境管理器，DI-engine在不同的工作子进程之间使用共享内存，以节省IPC的成本，`pyarrow <https://github.com/apache/arrow>`_在接下来的版本中会成为一个可靠的替代方案。

.. note::
   如果环境是某种客户端，像SC2和CARLA，也许基于python线程的新环境管理器可以更快。

.. note::
   如果在使用GPU的环境中存在一些预定义的神经网络，比如在RL训练前通过自监督训练的特征提取器VAE，DI-engine建议在每个子进程中利用并行执行，而不是在主进程中堆叠所有数据，然后转发这个网络。然而，这不是一个优雅的方法，DI-engine将试图找到一些新的灵活和通用的解决方案。

此外，为了保证实际使用中的鲁棒性，如IPC错误（断管、EOF）和环境运行时错误，DI-engine还提供了一系列的容错工具，例如：autodog(看门狗)和auto-retry(自动重试)。

对于所有提到的功能，用户可以参考 "EnvManager概述<.../feature/env_manager_overview_en.html>"了解更多细节。

策略
~~~~~~~
为了统一RL和其他机器学习算法的设计模式和模块化，DI-engine抽象并定义了具有多模式设计的通用策略接口。
有了这些抽象，大量的人工智能决策算法可以只在一个python文件中总结，即：对应的策略类。而用户的定制算法只需要继承和扩展:class:`Policy <ding.policy.Policy>`或者只是与它有相同的接口定义。

多模式策略
^^^^^^^^^^^^^^^^^^^^^^^^^^
在大多数情况下，RL策略需要为不同的用途执行不同的算法程序，例如：对于DQN，在训练中使用模型前进和计算TD误差。
模型在没有梯度计算的情况下进行前向传播，并使用epsilon-greedy来选择行动进行收集探索。因此，DI-engine策略将所有的算法内容统一在一个python文件中。
我们准备了一些简单的接口方法，并将它们组合成3种常用模式--**学习模式、收集模式、评估模式**，如下图所示。

.. image::
   images/policy_mode.svg

Learn_mode旨在进行策略更新，Collect_mode进行适当的探索和利用以收集训练数据，Eval_mode清晰而公平地评估策略性能。用户可以通过覆盖这些模式来定制自己的 用户可以通过覆盖这些模式来定制自己的算法思路，或者设计自己的定制模式，如根据训练结果进行超参数退火，在自我游戏训练中选择战斗队员等。了解更多细节,
...
共享模型+模型封装器
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
神经网络，通常称为模型，是整个算法中最重要的组成部分之一。对于串行管道，模型通常在公共的通用构造方法（``__init__``）中创建，或者从策略中出来，并作为参数传递给策略。因此，为了方便起见，模型在不同的模式中被共享。DI-engine通过 "模型封装器 "为模型扩展了更多的运行时功能，这使得共享的模型在不同的模式下可以表现出不同的行为，例如在收集模式下通过多叉分布进行采样，而在评估模式下则是:math:`argmax`。下面是一些具体的代码例子。
...
如果你想了解更多关于每种算法的预定义模型和定制你的模型，请参考`模型概述<.../feature/model_overview_en.html>`_。

如果你想了解预定义模型包装器的详细信息或定制你的模型包装器，请参考`包装器概述<.../feature/wrapper_hook_overview_en.html>`。

数据处理函数
^^^^^^^^^^^^^^^^^^^^^^
在实际的算法实现中，用户往往需要很多数据处理操作，比如将几个样本堆叠成一个批次，在torch.Tensor和np.ndarray之间进行数据转换。至于RL算法本身，有大量不同风格的数据预处理和聚合，如计算N步返回和GAE（Generalized Advantage Estimation），分割轨迹或解卷段，等等。从那时起，DI-engine提供了一些常用的处理函数，这些函数可以作为一个纯函数来调用。用户可以在收集模式和学习模式中使用这些功能。

例如，对于一些算法策略，如A2C/PPO，学习模式或收集模式，我们应该在哪里计算advantages？前者可以在分布式训练中把计算分配给不同的收集器节点，以节省时间，而后者由于更精确的近似，通常可以获得更好的性能。对于一个框架来说，提供一些强大而高效的工具比限制一些固定的管道更明智。下表显示了一些现有的处理功能和相关信息。
...
配置模块是一个供用户确定他们要使用何种参数的组件。
整体设计如下。

.. image:: images/config.png
   :alt: 

正如你在上图中看到的，整个配置主要由两部分组成。一个叫做Default Config，这是我们推荐的策略和环境的设置，可能不会有太大的变化。另一个叫做 "用户配置"，用户可能想逐一指定。

为了获得整个配置，我们有编译阶段，一个自下而上的过程。首先，我们得到每个子模块的默认设置，如学习者、收集器等。然后我们把它们放在一起，得到策略和环境的默认配置。最后，我们将默认配置与用户配置合并，得到整个配置。

另一方面，初始化阶段，即根据配置创建模块的过程，是自上而下的。我们将从策略和环境开始，然后将配置传递给每个执行模块。
...
在下表中，我们列出了一些常用的键以及它们的含义。
与策略有关的键，请参考文件`Hands On　RL <.../hands_on/index.html>`__部分。
...
-  用户特定的配置是最优先的，这意味着当冲突发生时，它将覆盖预定义的配置。

-  一些重要的键，如`env.stop_value', `env.n_evaluator_episode', `policy.on_policy`,　`policy.collect.n_sample'或`policy.collect.n_episode'必须是特定的。
...
在写完用户配置后，我们可以根据 "快速启动<.../quick_start/index.html>"运行我们的DQN实验。

Worker-收集器
~~~~~~~~~~~~~~~~~~
收集器是所有工作人员中最重要的组成部分之一，它在其他框架中通常被称为 "行为者"，DI-engine将其重新命名，以区别于行为者-批评者。它的目的是为政策培训（学习者）提供足够数量和质量的数据。收集器只负责收集数据，但与数据管理解耦，也就是说，它直接返回收集的轨迹，这些数据可以直接用于训练或推入重放缓冲区。

收集器有3个核心部分--环境管理器、策略（collect_mode）、收集器控制器，这些部分可以在一个进程中实现，也可以位于几个机器中。通常，DI-engine使用一个多进程的env_manager和另一个带有策略的主循环控制器进程来构建一个收集器，这在将来可能被扩展。

由于发送/接收数据的逻辑不同，现在的收集器分为两种模式--串行和并行，我们将分别介绍它们。

串行收集器
^^^^^^^^^^^^^^^^^^^
从收集数据的基本单位来看，样本sample（步骤）和剧集episode是两个主要使用的类型。因此，DI-engine为串行采集器定义了抽象接口`ISerialCollector'，并实现了`SampleCollector'和`EpisodeCollector'，这几乎涵盖了RL的使用，但当遇到一些特殊需求时，用户也可以轻松地进行定制。
...
.. note::
    对于所有的情况，收集数据的数量n_sample/n_episode，在整个训练过程中是固定的，所以我们的示例代码在配置中设置了这个字段，例如``config.policy.collect.n_sample``。


收集器的结构和主循环可以总结为下图，策略和环境的交互由`policy.forward', `env.step'和相关的支持代码组成。然后`policy.process_transition'和`policy.get_train_sample'有助于将数据处理成训练样本并打包成一个列表。对于`EpisodeCollector'，通常用于一些需要做特殊后处理的情况，`policy.get_train_sample'被禁用，用户在收到收集的数据后可以做任何事情。

.. image::
   images/collector_pipeline.svg
   :align: center

有时，我们使用不同的策略甚至不同的环境来收集数据，比如在训练开始时使用随机策略来准备初始数据，用专家策略的概率来计算损失函数。而所有的需求都可以通过 "reset_policy"、"reset_env"、"reset "这样的方法来实现。
...
此外，串行收集器在on-policy和off-policy算法之间显示出较少的差异，唯一的事情是重置一些统计数据和时间缓冲区，这可以由收集器自动执行，用户只需确保`config.policy.on_policy`的正确值即可。
...
缓冲器是一个组件，用于存储由收集器收集的数据或由固定策略（通常是专家策略）产生的数据，然后为学习者提供数据以优化策略。在DI-engine中，有３种类型的缓冲器。
...
``AdvancedReplayBuffer`` 实现了以下功能(如图所示)：
   ...
   - **吞吐量的监测和控制**。在一个固定的时间段内，计算有多少数据被推入、抽出或移出缓冲区。在一定范围内控制 "推入"/"抽出 "的比例，以防数据流速度不匹配。
...
环境管理器允许我们一个一个地运行多个环境（"base_env_manager"）或并行地运行（"subprocess_env_manager"）。例如，如果我们使用子进程环境管理器，我们将在不同的子进程中运行不同的环境，这将大大增加收集事件的效率。

Policy(eval_mode)是我们需要检查的rl模型。

评价器控制器是一个决定我们是否应该停止评价的组件。例如，在 "Serial Evaluator "中，"n_evaluator_episode "是一个参数，用来确定我们要收集和评价多少个episode。一旦我们收集了这些数量的episode，评估器将停止收集并开始计算平均奖励。如果平均数大于`stop_value'，`stop_flag'将为True，我们将知道我们的模型已经收敛了。
...
.. tip::
   **如何判断模型是否已经收敛**
   ...
   此外，一个可靠的RL实验应该用不同的随机种子重复3~5次，一些统计数据如中位数值和平均值/std值可以更有说服力。
...
.. tip::
   ...
   **如何解决评价器中的不同环境可能启动不同长度episode的问题？**
   ...
   为了解决这个问题，我们使用 "VectorEvalMonitor"，一个组件来平衡每个环境要收集多少个情节。让我们回到上面的例子，我们将为前两个环境中的任何一个收集三个情节，但对其余环境中的每一个只收集两个。
...
**并行模式**将调用学习者的`start'方法进行完整的训练过程。`start'方法有一个循环，包括从源头（通常是文件系统）获取数据，并调用`train'进行一次迭代训练。`start'将训练特定的迭代次数，这是由使用配置设置的。
