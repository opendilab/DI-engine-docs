...
离线强化学习（RL）是一个新出现的研究领域，旨在使用大量先前收集的数据集来学习行为，而无需进一步与环境进行交互。
...
是其中之一，它通过添加更多评价网络来惩罚分布外（OOD）操作。
...
重要公式/重要图示
---------------------------
EDAC 表明，通过增加批评家的数量，可以通过剪切 Q 学习（clipped Q-learning） :math:`min_{j=1,2}Q(s,a)` 来惩罚 OOD 操作。 因此，EDAC 可以通过在标准 SAC RL 算法上集成 Q 网络并添加惩罚项来惩罚 OOD 操作来实现。
