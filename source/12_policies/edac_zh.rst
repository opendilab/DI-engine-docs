...
离线强化学习（RL）是一个重新出现的研究领域，旨在使用大量先前收集的数据集来学习行为，而无需进一步与环境进行交互。
它有可能在许多实际决策问题中取得巨大进展，其中主动数据收集昂贵（例如，在机器人，药物发现，对话生成，推荐系统中）或危险（例如，医疗保健，自动驾驶或教育）。 
此外，在线收集的数据量远远低于离线数据集。这样的范式有望解决将强化学习算法从受限实验室环境带到现实世界的关键挑战。

然而，在离线设置中直接使用现有的基于价值的离策略 RL 算法通常会导致性能不佳，这是由于从分布外操作引导和过度拟合等问题。
因此，许多约束技术（例如政策约束；保守估计；不确定性估计）被添加到基本在线 RL 算法中。 基于不确定性的离线强化学习与多
...
是其中之一，它通过添加更多批评网络来惩罚分布外（OOD）操作。

快速了解
---------
1. EDAC 是一种离线不确定性估计 RL 算法。

2. EDAC 可以在许多 SAC RL 算法之上用不到20行代码实现

3. EDAC 支持连续动作空间。

重要公式/重要图示
---------------------------
EDAC 表明，通过增加批评家的数量，可以通过剪切 Q 学习 :math:`min_{j=1,2}Q(s,a)` 来惩罚 OOD 操作。 因此，EDAC 可以通过在标准 SAC RL 算法上集成 Q 网络并添加惩罚项来惩罚 OOD 操作来实现。
...
通过添加上述惩罚项，该算法可以最小化实现更高计算效率的必要批评家数量，同时保持良好的性能。该项计算数据集状态操作的 Q 值梯度的内积。

EDAC 通过增加 SAC 算法中 Q 网络的数量来显示剪切 Q 学习的重要性。通过计算 clipe 惩罚和 OOD 操作和数据集操作的标准偏差，论文显示为什么算法通过增加
Q 网络的数量会表现得更好。这些结果的图形如下：
...
。这进一步解释了为什么增加批评家数量可以惩罚 OOD 操作，并且也指示了论文后期改进的方向。

.. image:: images/edac_Q_grad.png
   :align: center
   :scale: 55%

尽管增加 Q 网络的数量可以提高性能，但过多的 Q 网络将成为负担。因此，EDAC 添加了惩罚项以减少批评家数量。 使用一阶泰勒近似，沿 w 的 OOD 操作处 Q 值的样本方差可以表示如下：

.. image:: images/edac_taylor.png
   :align: center
   :scale: 55%

为了有效地增加近分布 OOD 操作的 Q 值方差，EDAC 最大化以下方程：
...
的总和，这等于总方差。 上述方程等同于最小化第一幅图的方程。

伪代码
---------------
伪代码显示在算法1中，与传统的演员评论家算法（例如 SAC）的区别以蓝色显示。
