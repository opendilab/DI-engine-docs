...
Q函数引导的策略优化算法，即 Q-Guided Policy Optimization(QGPO), 由路橙、陈华玉等，发表于 2023 年的论文 `《Contrastive Energy Prediction for Exact Energy-Guided Diffusion Sampling in Offline Reinforcement Learning》 <https://arxiv.org/abs/2304.12824>`_ 中,
它是一种基于能量式条件扩散模型的 actor-critic 离线强化学习算法.

QGPO 算法由三个关键部分组成：无条件扩散模型的行为策略、动作状态价值模型，以及扩散中间态的能量模型，它用于引导最优策略的条件扩散模型的生成。

论文提出了能量条件扩散模型的新的训练目标，称之为对比能量预测 (CEP)。CEP 是一种对比学习目标，其关注的是在相同的状态行为搭配之间，最大化能量函数和能量引导的互信息。

通过使用离线数据集训练扩散模型，可以得到无条件扩散模型。而使用基于贝尔曼方程的训练目标，可以得到动作状态价值模型。
...
关键方程或关键框图
---------------------------
强化学习算法中的约束策略优化的最优策略 :math:`\pi^*` 满足：
...
它可被视为以 :math:`-Q_{\psi}(s,a)` 为能量函数， :math:`\beta` 为温度系数，关于动作 :math:`a` 的Boltzmann 分布。
...
+---------------------+-----------------+----------------+---------------+----------+----------+
| 环境                |random           |medium replay   |medium expert  |medium    |expert    |
+=====================+=================+================+===============+==========+==========+
|                     |                 |                |               |          |          |
|Halfcheetah          |                 |                |11226          |          |          |
|                     |                 |                |               |          |          |
